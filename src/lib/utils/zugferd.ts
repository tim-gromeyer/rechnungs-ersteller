...
const container = (name: string, elements: (Element | undefined)[], attributes?: Record<string, string>): Element => ({
	type: 'element',
	name,
	attributes,
...
export function generateZugferdXml(invoice: Invoice): string {
	...
	const json: Element = container('rsm:CrossIndustryInvoice', [
		...
		container('rsm:SupplyChainTradeTransaction', [
			container('ram:ApplicableHeaderTradeAgreement', [
				container('ram:SellerTradeParty', [
					...
					sender.taxId ? container('ram:SpecifiedTaxRegistration', [el('ram:ID', sender.taxId, { schemeID: 'FC' })]) : undefined,
					sender.vatId ? container('ram:SpecifiedTaxRegistration', [el('ram:ID', sender.vatId, { schemeID: 'VA' })]) : undefined
				]),
			...
			container('ram:ApplicableHeaderTradeSettlement', [
				...
				container('ram:SpecifiedTradePaymentTerms', [
					container('ram:DueDateDateTime', [el('udt:DateTimeString', dueDateStr, { format: '102' })])
				])
			]),
			...
			...discounts.map((discount, index) =>
				container('ram:IncludedSupplyChainTradeLineItem', [
					container('ram:AssociatedDocumentLineDocument', [el('ram:LineID', articles.length + index + 1)]),
