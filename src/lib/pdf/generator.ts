...
const pdfMakeInstance = (pdfMake as unknown as { default: typeof pdfMake }).default || pdfMake;
const pdfFontsInstance =
	...
	(pdfFonts as unknown as { default: Record<string, string> }).default ||
	(pdfFonts as unknown as Record<string, string>);
(pdfMakeInstance as unknown as { vfs: Record<string, string> }).vfs = pdfFontsInstance;
...
function toArrayBuffer(input: unknown): ArrayBuffer {
	...
	return new Uint8Array(input as ArrayLike<number>).buffer as ArrayBuffer;
}
...
export async function generatePdf(
	...
	totalsBody.push([
		{
			...
			// @ts-expect-error - colSpan exists but might not be in the basic Content type
			colSpan: 2,
			margin: [0, 6, 0, 6] as Margins
		},
		'' // Empty string instead of empty object
	]);
	...
	const docDefinition: TDocumentDefinitions = {
		...
		pageMargins: [45, 35, 45, 35] as Margins,
		content: content,
		styles: {
