...
<Card.Root
	class="invoice-preview mx-auto flex min-h-[297mm] w-[210mm] flex-shrink-0 flex-col bg-white px-[15mm] py-[10mm] font-sans text-[11px] leading-tight text-black ring-1 shadow-xl ring-black/5 print:m-0 print:min-h-0 print:w-full print:ring-0 print:shadow-none"
>
		...
		<div class="mb-6 flex justify-end">
			<div class="space-y-0.5 text-right text-[9px] text-gray-600">
				...
				{#if invoice.sender.taxId}
					<div class="pt-0.5">Steuernr.: {invoice.sender.taxId}</div>
				{/if}
		...
		<div>
				...
				<tfoot>
					...
					<tr>
						<td colspan="2" class="py-0.5 text-gray-700">
							zzgl. MwSt. {invoice.settings.vatRate}%</td
						>
			...
			<div class="mt-4 text-[10px] text-gray-700 print:break-inside-avoid">
				...
				{#if invoice.settings.vatRate === 0 && invoice.settings.taxNote}
					<p class="mb-2 text-[9px] text-gray-600 italic">
